...
original_values = ['aseas', 'cance', 'di5gv', 'dsmto', 'egunb', 'hed4e', 'lou0g', 'mlhdi', 'nized', 'ofror', 'opmen', 'owso9', 'sso55', 'tpoin', 'tthel']
def find_answer_5():
...
def add_one_salt(list_to_hash):
    ...
    for lines in list_to_hash:
        ...
        new_hashed = hashlib.new('md5')
        # Unicode-objects must be encoded before hashing
        encoded_combo = lines.encode('utf-8')
        # need to update after encoding
        new_hashed.update(encoded_combo)
        hasehd_value = new_hashed.hexdigest()
...
# rainbow table
#time for look up the salted hash value is 1min 50 sec
# time for look up the original 5 letters hash value is 50 secs
result1 = add_one_salt(original_values)
